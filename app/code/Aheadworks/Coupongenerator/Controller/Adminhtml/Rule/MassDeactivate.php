 ...
 */
class MassDeactivate extends \Aheadworks\Coupongenerator\Controller\Adminhtml\Rule\MassAbstract
{
    /**
     * @var string
     */
    protected $errorMessage = 'Something went wrong while deactivating rule(s).';
    ...
    protected function massAction($collection)
    {
        ...
        foreach ($collection->getItems() as $rule) {
            ...
            if ($magentoSalesRule->getRuleId()) {
                $magentoSalesRule->setIsActive(\Aheadworks\Coupongenerator\Model\Source\Rule\Status::STATUS_INACTIVE);
                $this->magentoSalesRuleRepository->save($magentoSalesRule);
                $count++;
            }
        }
        $this->messageManager->addSuccessMessage(__('A total of %1 rule(s) have been deactivated', $count));
    }
}

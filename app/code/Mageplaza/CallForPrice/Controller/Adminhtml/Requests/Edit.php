<?php
/**
 * Mageplaza
 *
 * NOTICE OF LICENSE
 *
 * This source file is subject to the Mageplaza.com license that is
 * available through the world-wide-web at this URL:
 * https://www.mageplaza.com/LICENSE.txt
 *
 * DISCLAIMER
 *
 * Do not edit or add to this file if you wish to upgrade this extension to newer
 * version in the future.
 *
 * @category    Mageplaza
 * @package     Mageplaza_CallForPrice
 * @copyright   Copyright (c) Mageplaza (https://www.mageplaza.com/)
 * @license     https://www.mageplaza.com/LICENSE.txt
 */

namespace Mageplaza\CallForPrice\Controller\Adminhtml\Requests;

use Mageplaza\CallForPrice\Controller\Adminhtml\Requests;

/**
 * Class Edit
 * @package Mageplaza\CallForPrice\Controller\Adminhtml\Requests
 */
class Edit extends Requests
{
    /**
     * @return \Magento\Framework\App\ResponseInterface|\Magento\Framework\Controller\ResultInterface|void
     */
    public function execute()
    {
        $requestId = $this->getRequest()->getParam('request_id');
        $request   = $this->_initRequest();

        if (!$request->getRequestId() && $requestId) {
            $this->messageManager->addError(__('This request no longer exists.'));
            $this->_redirect('mpcallforprice/*/');

            return;
        }

        $data = $this->_getSession()->getData('callforprice_request_data', true);
        if (!empty($data)) {
            /** convert store data*/
            $storeData         = explode(',', $data['store_ids']);
            $data['store_ids'] = $storeData;

            $request->addData($data);
        }

        $this->_view->loadLayout();
        $this->_setActiveMenu('Mageplaza_CallForPrice::requests');
        $title = $request->getRequestId() ? $request->getName() : __('New Request');
        $this->_view->getPage()->getConfig()->getTitle()->prepend(__($title));

        $this->_view->renderLayout();
    }
}

...
class UpgradeSchema implements UpgradeSchemaInterface
{
    ...
    public function upgrade(SchemaSetupInterface $setup, ModuleContextInterface $context)
    {
        ...
        if ($context->getVersion() && version_compare($context->getVersion(), '1.5.0', '<')) {
            $this->addQuickResponseTables($setup);
        }
    }
    ...
    /**
     * Add quick responses tables
     *
     * @param SchemaSetupInterface $installer
     */
    private function addQuickResponseTables(SchemaSetupInterface $installer)
    {
        /**
         * Create table 'aw_helpdesk_quick_response'
         */
        $table = $installer->getConnection()
            ->newTable($installer->getTable('aw_helpdesk_quick_response'))
            ->addColumn(
                'id',
                ...
                'Quick Response Modification Time'
            )->addColumn(
                'is_active',
                ...
                $installer->getIdxName(
                    $installer->getTable('aw_helpdesk_quick_response'),
                    ['title'],
                    \Magento\Framework\DB\Adapter\AdapterInterface::INDEX_TYPE_FULLTEXT
                ),
                ['title'],
                ['type' => \Magento\Framework\DB\Adapter\AdapterInterface::INDEX_TYPE_FULLTEXT]
            )->setComment('Quick Response');
        $installer->getConnection()->createTable($table);

        /**
         * Create table 'aw_helpdesk_quick_response_text'
         */
        $table = $installer->getConnection()
            ->newTable($installer->getTable('aw_helpdesk_quick_response_text'))
            ->addColumn(
                'response_id',
                \Magento\Framework\DB\Ddl\Table::TYPE_INTEGER,
                null,
                ...
                $installer->getIdxName('aw_helpdesk_quick_response_text', ['response_id']),
                ['response_id']
            )->addIndex(
                $installer->getIdxName('aw_helpdesk_quick_response_text', ['store_id']),
                ['store_id']
            )->addForeignKey(
                $installer->getFkName(
                    'aw_helpdesk_quick_response_text',
                    'response_id',
                    'aw_helpdesk_quick_response',
                    'id'
                ),
                'response_id',
                $installer->getTable('aw_helpdesk_quick_response'),
                ...
                $installer->getFkName('aw_helpdesk_quick_response_text', 'store_id', 'store', 'store_id'),
                'store_id',
                $installer->getTable('store'),
                'store_id',
                \Magento\Framework\DB\Ddl\Table::ACTION_CASCADE
            )->setComment('AW Helpdesk Quick Response Text Table');
        $installer->getConnection()->createTable($table);
    }
}

...
class Portfolio extends Template
{
    ...
    protected function _prepareLayout()
    {
		...
		$title = ($portfolio->getData('seo_title'))? $portfolio->getData('seo_title') : $portfolio->getName();
		$keywords = $portfolio->getData('seo_keywords');
		$description = $portfolio->getData('seo_descriptions');
	...
	public function getBaseImage($portfolio){
		$filePath = $portfolio->getBaseImage();
		if($filePath!=NULL){
			$imageUrl = $this->_urlBuilder->getBaseUrl(['_type' => \Magento\Framework\UrlInterface::URL_TYPE_MEDIA]) . 'mgs/portfolio/image/'.$filePath;
			return $imageUrl;
		}
		return 0;
	}

	public function getGalleryImage1($portfolio){
		$filePath = $portfolio->getData('gallery_image1');
		if($filePath!=NULL){
			$imageUrl = $this->_urlBuilder->getBaseUrl(['_type' => \Magento\Framework\UrlInterface::URL_TYPE_MEDIA]) . 'mgs/portfolio/image/'.$filePath;
			return $imageUrl;
		}
	...
	public function getGalleryImage2($portfolio){
		$filePath = $portfolio->getData('gallery_image2');
		if($filePath!=NULL){
			$imageUrl = $this->_urlBuilder->getBaseUrl(['_type' => \Magento\Framework\UrlInterface::URL_TYPE_MEDIA]) . 'mgs/portfolio/image/'.$filePath;
			return $imageUrl;
		}
	...
	public function getGalleryImage3($portfolio){
		$filePath = $portfolio->getData('gallery_image3');
		if($filePath!=NULL){
			$imageUrl = $this->_urlBuilder->getBaseUrl(['_type' => \Magento\Framework\UrlInterface::URL_TYPE_MEDIA]) . 'mgs/portfolio/image/'.$filePath;
			return $imageUrl;
		}
	...
	public function getGalleryImage4($portfolio){
		$filePath = $portfolio->getData('gallery_image4');
		if($filePath!=NULL){
			$imageUrl = $this->_urlBuilder->getBaseUrl(['_type' => \Magento\Framework\UrlInterface::URL_TYPE_MEDIA]) . 'mgs/portfolio/image/'.$filePath;
			return $imageUrl;
		}
	...
	public function getGalleryImage5($portfolio){
		$filePath = $portfolio->getData('gallery_image5');
		if($filePath!= NULL){
			$imageUrl = $this->_urlBuilder->getBaseUrl(['_type' => \Magento\Framework\UrlInterface::URL_TYPE_MEDIA]) . 'mgs/portfolio/image/'.$filePath;
			return $imageUrl;
		}

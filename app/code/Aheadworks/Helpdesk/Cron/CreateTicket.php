...
class CreateTicket
{
    ...
    public function execute()
    {
        ...
        foreach ($mailCollection as $mail) {
            ...
            $messageType = \Aheadworks\Helpdesk\Model\ThreadMessage::OWNER_CUSTOMER_VALUE;
            $authorName = $ticket->getCustomerName();
            $authorEmail = $ticket->getCustomerEmail();

            //check if mail from agent
            if ($ticket->getAgentId()) {
                $agent = $this->agentFactory->create();
                $this->agentResource->load($agent, $ticket->getAgentId());
                $from = $this->parseEmail($mail->getFrom());
                if ($agent && $agent->getIsActive() && $agent->getEmail() == $from) {
                    $messageType = \Aheadworks\Helpdesk\Model\ThreadMessage::OWNER_ADMIN_VALUE;

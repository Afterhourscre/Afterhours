...
use Magento\MediaStorage\Model\File\Uploader as MediaStorageFileUploader;
...
class Attachment extends \Magento\Framework\Model\AbstractModel
{
    ...
    public function beforeSave()
    {
        ...
        $filePath = $mediaDirectory->getAbsolutePath(static::TMP_PATH);
        $fileName = MediaStorageFileUploader::getCorrectFileName($this->getFile());
        $fileName = $filePath . $fileName;
        if ($mediaDirectory->isExist($fileName)) {
            if ($this->getRemoved()) {
                $this->_dataSaveAllowed = false;
            } else {
                $fileContent = $mediaDirectory->readFile($fileName);

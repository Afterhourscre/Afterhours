...
class Price extends AbstractFilter
{
    ...
    public function getSliderConfig()
    {
        ...
        if ($this->_filterVal) {
            $this->resetPriceFilters($productCollection);
        }
    ...
    /**
     * Reset price filters for a product collection
     *
     * @param \Magento\CatalogSearch\Model\ResourceModel\Fulltext\Collection $collection
     * @return void
     */
    private function resetPriceFilters($collection)
    {
        $select = $collection->getSelect();
        $where = $select->getPart(\Zend_Db_Select::WHERE);
        foreach ($where as $key => $condition) {
            if (strpos($condition, 'price') !== false) {
                unset($where[$key]);
            }
        }

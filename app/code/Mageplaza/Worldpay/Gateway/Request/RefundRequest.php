...
class RefundRequest extends AbstractRequest implements BuilderInterface
{
    ...
    public function build(array $buildSubject)
    {
        ...
        $txnId   = $payment->getAdditionalInformation(PaymentInfo::TXN_ID);

        return [
            'url'          => str_replace('%txn_id%', $txnId, Request::REFUND_URL),
            'refundAmount' => $this->helper->convertAmount($buildSubject['amount'], $payment->getOrder())
        ];
    }
}

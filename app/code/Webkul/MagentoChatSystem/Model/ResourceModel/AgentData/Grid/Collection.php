...
class Collection extends AgentDataCollection implements SearchResultInterface
{
    ...
    /**
     * Join admin_user_session relation table
     *
     * @return void
     */
    protected function _renderFiltersBefore()
    {
        $sessionIds = $this->getAgentCurrentStatus();
        if (!empty($sessionIds)) {
            $adminSessionTable = $this->getTable('admin_user_session');
            $this->getSelect()->join(
                $adminSessionTable.' as aus',
                'main_table.agent_id = aus.user_id',
                ['status' => 'status']
            )->where('aus.id IN('.implode(',', $sessionIds).')');
        }
     ...
     * @return array
     */
    protected function getAgentCurrentStatus()
    {
        $sessionIds = [];
        $ids = $this->getAllAgentIds();
        ...
        foreach ($sessionCollection as $model) {
            if (!in_array($model->getUserId(), $loggedInIds) && $model->getStatus() == 1) {
                $model->setStatus(0)->save();
            }
            $sessionIds[] = $model->getId();
        }
        return $sessionIds;
    }

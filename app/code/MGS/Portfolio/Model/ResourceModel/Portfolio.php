...
class Portfolio extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
    ...
    protected function _afterSave(\Magento\Framework\Model\AbstractModel $object)
    {
        ...
        if(isset($_FILES['gallery_image1']['name']) && $_FILES['gallery_image1']['name'] != '') {
            try {
                $uploader = $this->_fileUploaderFactory->create(['fileId' => 'gallery_image1']);
                $uploader->setAllowedExtensions(['jpg', 'jpeg', 'gif', 'png']);
                $uploader->setAllowRenameFiles(true);
                $uploader->setFilesDispersion(true);
            ...
            $fileName = $uploader->getUploadedFileName();
            if ($fileName) {
                $object->setData('gallery_image1', $fileName);
                $object->save();
            }
        }

        if(isset($_FILES['gallery_image2']['name']) && $_FILES['gallery_image2']['name'] != '') {
            try {
                $uploader = $this->_fileUploaderFactory->create(['fileId' => 'gallery_image2']);
                $uploader->setAllowedExtensions(['jpg', 'jpeg', 'gif', 'png']);
                $uploader->setAllowRenameFiles(true);
                $uploader->setFilesDispersion(true);
            ...
            $fileName = $uploader->getUploadedFileName();
            if ($fileName) {
                $object->setData('gallery_image2', $fileName);
                $object->save();
            }
        }

        if(isset($_FILES['gallery_image3']['name']) && $_FILES['gallery_image3']['name'] != '') {
            try {
                $uploader = $this->_fileUploaderFactory->create(['fileId' => 'gallery_image3']);
                $uploader->setAllowedExtensions(['jpg', 'jpeg', 'gif', 'png']);
                $uploader->setAllowRenameFiles(true);
                $uploader->setFilesDispersion(true);
            ...
            $fileName = $uploader->getUploadedFileName();
            if ($fileName) {
                $object->setData('gallery_image3', $fileName);
                $object->save();
            }
        }

        if(isset($_FILES['gallery_image4']['name']) && $_FILES['gallery_image4']['name'] != '') {
            try {
                $uploader = $this->_fileUploaderFactory->create(['fileId' => 'gallery_image4']);
                $uploader->setAllowedExtensions(['jpg', 'jpeg', 'gif', 'png']);
                $uploader->setAllowRenameFiles(true);
                $uploader->setFilesDispersion(true);
            ...
            $fileName = $uploader->getUploadedFileName();
            if ($fileName) {
                $object->setData('gallery_image4', $fileName);
                $object->save();
            }
        }

        if(isset($_FILES['gallery_image5']['name']) && $_FILES['gallery_image5']['name'] != '') {
            try {
                $uploader = $this->_fileUploaderFactory->create(['fileId' => 'gallery_image5']);
                $uploader->setAllowedExtensions(['jpg', 'jpeg', 'gif', 'png']);
                $uploader->setAllowRenameFiles(true);
                $uploader->setFilesDispersion(true);
            ...
            $fileName = $uploader->getUploadedFileName();
            if ($fileName) {
                $object->setData('gallery_image5', $fileName);
                $object->save();
            }
        }

...
class Order
{
    ...
    public function getOptionArrayByCustomerData($customerId, $customerEmail)
    {
        ...
        if ($customerId) {
            $whereCondition = "(main_table.customer_id = {$customerId} OR "
                . "main_table.customer_email = '{$customerEmail}')";
        } else {
        ...
        $unassignedLabel = self::UNASSIGNED_LABEL;
        $result = [self::UNASSIGNED_VALUE => __($unassignedLabel)];

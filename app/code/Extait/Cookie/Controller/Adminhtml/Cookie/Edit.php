...
class Edit extends AbstractController
{
    ...
    public function execute()
    {
        ...
        // Register cookie if isset cookie ID. That use for handing delete button.
        if (isset($cookieID)) {
            $cookie = $this->cookieFactory->create();
            $this->cookieResourceModel->load($cookie, $cookieID);
            $this->registry->register('current_cookie', $cookie);
        }

        /** @var \Magento\Backend\Model\View\Result\Page $resultPage */
        $resultPage = $this->resultFactory->create(ResultFactory::TYPE_PAGE);
        $resultPage->setActiveMenu('Extait_Cookie::cookie');
        $resultPage->getConfig()->getTitle()->prepend((__('Edit Cookie')));

...
class Automation extends \Magento\Framework\Model\AbstractModel
{
     ...
     * @param \Aheadworks\Helpdesk\Model\Source\Ticket\Agent
     * @param array $data
    ...
    {
        $this->_init(\Aheadworks\Helpdesk\Model\ResourceModel\Automation::class);
    }
    ...
    public function getValidatedTicketIds()
    {
        ...
        foreach ($this->getConditions() as $condition) {
            ...
            if ($condition['object'] == \Aheadworks\Helpdesk\Model\Source\Automation\Condition::CUSTOMER_GROUP_VALUE
            ...
            if ($condition['object'] == \Aheadworks\Helpdesk\Model\Source\Automation\Condition::LAST_REPLIED_HOURS_VALUE
            ) {
    ...
    private function sendEmailToAgent($template, $messageId = null)
    {
        ...
        $backendUrl = $this->backendUrlBuilder->getUrl(
        ...
        if ($gateway && $gateway->getIsEnabled() && $gateway->getEmail()) {
            $emailData['gateway'] = $gateway->getEmail();
            $needReplyTo = true;
        }
        $this->sender->sendEmail($emailData, $needReplyTo);
        return $this;
    }
    ...
    /**
     * Check is agent available
     * @param int $agentId
     * @return bool
     */
    private function isAgentAvailable($agentId)
    {
        $adminUser = $this->agentFactory->create();
        $this->agentResource->load($adminUser, $agentId);
        $availableAgents = $this->agentSource->getAvailableAgents();

        if (!$adminUser
            || !$adminUser->getId()
            || !$adminUser->getIsActive()
            || (!empty($availableAgents) && !in_array($adminUser->getId(), $availableAgents))
        ) {
            return false;
        }
        return true;

...
class Replace extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    /**
     * @var \Mirasvit\SeoToolbar\Model\Config
     */
    ...
    public function __construct(
        ...
        \Mirasvit\SeoToolbar\Model\Config $toolbarConfig
    ) {
    ...
    public function addLinks($text)
    {
        ...
        $text  = $this->_addLinks($text, $links, $patternsForExclude);
    ...
    public function _addLinks($text, $links, $excludedTags, $replacementCountForTests = false)
    {
        ...
        foreach ($links as $link) {
            ...
            if ($this->toolbarConfig->isToolbarAllowed()) {
                $replaceLimit = 'data-link-limit="'. ($link->getMaxReplacements() ? $link->getMaxReplacements():'unlimited') .'"';
                $limitPerPage = 'data-page-limit="'. ($this->getMaxLinkPerPage() ? $this->getMaxLinkPerPage():'unlimited') .'"';
            }

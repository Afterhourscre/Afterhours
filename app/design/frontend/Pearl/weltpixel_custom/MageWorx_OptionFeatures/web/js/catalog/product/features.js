            ...
            for (var option_id in optionConfig) {
                ...
                if (this.options.option_description_enabled && !_.isEmpty(extendedOptionsConfig[option_id]['description'])) {
                    if (this.options.option_description_mode == this.options.option_description_modes.tooltip) {
                        ...
                        if ($label.length > 0) {
                            var $labelText = description.split('+')[0],
                                $tooltipLink = description.split('+')[1];
                            $label
                                .first()
                                .after($('<div class="tooltip wrapper option-description-text"><a href="/' + $tooltipLink + '" target="_blank"><span class="icon-question-sign" aria-hidden="true" style="font-size: 20px;"></span></a><span class="item options tooltip content"><p>'+ $labelText + '</p></span></div>'));
                        } else {
